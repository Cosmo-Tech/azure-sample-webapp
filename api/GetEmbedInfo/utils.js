...
const sanitizeAndValidateConfig = () => {
  ...
  const keycloakRealm = getConfigValue('KEYCLOAK_REALM');
  if (azureCsmApiAppId && keycloakRealm)
    throw new ServiceAccountError(
      500,
      'Configuration error',
      "Can't define both parameters AZURE_COSMO_API_APPLICATION_ID and KEYCLOAK_REALM. Please check the PowerBI " +
  ...
  if (!azureCsmApiAppId && !keycloakRealm)
    throw new ServiceAccountError(
      500,
      'Configuration error',
      'You must define one of parameters AZURE_COSMO_API_APPLICATION_ID or KEYCLOAK_REALM. Please check the PowerBI ' +
        'service account configuration.'
    );
};
