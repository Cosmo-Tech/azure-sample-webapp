...
describe('Dataset Manager - Parameters Edition', () => {
  ...
  it('should edit ETL parameters of a dataset', () => {
    ...
    const runnerETLDynamicValues = RUNNERS[0];
    const datasetETLLocalFile = DATASETS[4];
    const runnerETLLocalFile = RUNNERS[1];
    ...
    DatasetManager.getRunnerRunTemplate().should('exist').contains(runnerETLDynamicValues.runTemplateId);
    ...
    DatasetManager.getUpdateDatasetParametersButton().should('be.visible');
    DatasetManager.openUpdateDatasetParametersDialog();
    DatasetManager.getRunnerRunTemplate().should('exist').contains(runnerETLLocalFile.runTemplateId);
