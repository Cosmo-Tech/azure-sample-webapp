...
applying changes to the webapp configuration, based on **environment variables** and **configuration files** that
can be fetched from a git repository.
...
```
usage: patch_webapp_server.py [-h] [-n] [-e ENV_FILE] -i INPUT_FOLDER [-o OUTPUT_FOLDER]
                              [--csp CSP [CSP ...]]
...
`python patch_webapp_server.py -n -i my_custom_config_folder`
...
- `custom`: will search for the file _csp.json_ in the input configuration folder, and add its content
...
keep in your custom _csp.json_ file.
