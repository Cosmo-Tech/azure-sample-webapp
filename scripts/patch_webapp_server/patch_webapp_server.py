...
import sys
import argparse
# import git  # TODO: allow usage of git repo as input configuration folder
import json
import fileinput
...
def parse_arguments():
    ...
    # parser.add_argument("-s", "--source-repository",
    #                     help="URL of the git repository to clone, to use as configuration input, fetch public" +
...
def apply_csp(output_folder, csp_html):
    # TODO: inject resulting CSP into index.html
...
def load_config_values(input_config_folder, env_file):
    # TODO: handle loading from env variables if values exist
    # TODO: handle loading from env file if enabled
    config_values = {}
    file_names = ["ApplicationInsights.json", "GlobalConfiguration.json", "HelpMenuConfiguration.json"]
    for file_name in file_names:
        new_values = load_json(os.path.join(input_config_folder, 'config', file_name))
...
def apply_config_values(output_folder, js_env_file_content, public_url=""):
    ...
    html_file_path = os.path.join(output_folder, 'index.html')
    pattern = '<script id="publicWebappConfig"></script>'
    script_tag = f'<script src="{public_url}/static/js/publicWebappConfig.js"></script>'
...
def main():
    ...
    # Safety checks
    # TODO: specify how to fail with available logs & stop the container if required
    if False:
        sys.exit(1)

    # TODO: implement switch for github repo input
    ...
    # TODO: fill CSP with API URL hostname
    ...
    # Generate changes for translation.json
    ...
    apply_config_values(args.output_folder, js_env_file_content)
    replace_translation_files(args.output_folder, new_translation_files)
    #  - modify index.html (replace application name in page title)
