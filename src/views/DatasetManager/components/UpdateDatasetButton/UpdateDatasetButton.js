...
import { useGetAllRunners } from '../../../../state/hooks/RunnerHooks';
import { UpdateDatasetDialog } from './components';

export const UpdateDatasetButton = ({ dataset }) => {
  const { t } = useTranslation();
  const runners = useGetAllRunners();
  const selectedRunner = runners.find((runner) => runner.id === dataset.source.name);

  const isDisabled =
    (dataset && dataset.ingestionStatus === INGESTION_STATUS.PENDING) ||
    !selectedRunner ||
    Object.keys(selectedRunner ?? {})?.length === 0;
