...
const ModeSwitcher = ({ mode, onChange }) => {
  ...
  const useStyles = makeStyles({
    container: {
      ...
      margin: '0 auto',
      width: 'fit-content',
      backgroundColor: simulationTheme[paletteMode].button.background,
      borderRadius: 8,
      display: 'flex',
      gap: 8,
    },
    toggleButton: {
      color: simulationTheme[paletteMode].button.color,
      border: 'none',
      borderRadius: 8,
      textTransform: 'none',
      fontSize: 16,
      ...
      gap: 8,
      '&.Mui-selected': {
        backgroundColor: simulationTheme[paletteMode].button.hoverBackground,
        color: simulationTheme[paletteMode].button.color,
      },
      '&:hover': {
        backgroundColor: simulationTheme[paletteMode].button.hoverBackground,
  ...
  const getIconColor = (currentMode, buttonMode) =>
    currentMode === buttonMode
      ? simulationTheme[paletteMode].button.icon.currentMode
      : simulationTheme[paletteMode].button.icon.buttonMode;
