...
export const MapEntitiesDropdown = () => {
  ...
  const handleChange = (event) => {
    ...
    const switchValues = selectedMapFilters.filter((v) =>
      MAP_VIEW_FILTERS.some((f) => f.type === 'switch' && f.value === v)
    );

    const checkboxValues = MAP_VIEW_FILTERS.filter((f) => f.type === 'checkbox').map((f) => f.value);

    const lastClicked =
      value.find((v) => !selectedMapFilters.includes(v)) || selectedMapFilters.find((v) => !value.includes(v));

    const addAllIfNeeded = (arr) => {
      const selectedCheckboxCount = arr.filter((cv) => cv !== 'onlyCriticalPoints').length;
      return selectedCheckboxCount === checkboxValues.length - 1
        ? Array.from(new Set([...arr, 'all']))
        : Array.from(new Set(arr));
    };

    if (lastClicked === 'all') {
      const newValues = selectedMapFilters.includes('all')
        ? [...switchValues]
        : [...switchValues, ...checkboxValues, 'all'];

      setSelectedMapFilters(Array.from(new Set(newValues)));
      return;
    }

    if (selectedMapFilters.includes('all')) {
      const updated = selectedMapFilters.filter((v) => v !== 'all' && v !== lastClicked);
      const finalValues = value.includes(lastClicked)
        ? [...updated, lastClicked, ...switchValues]
        : [...updated, ...switchValues];

      setSelectedMapFilters(addAllIfNeeded(finalValues));
      return;
    }

    const final = [...value, ...switchValues.filter((v) => !value.includes(v))];
    setSelectedMapFilters(addAllIfNeeded(final));
  };
  ...
  const handleRemove = (value) => {
    ...
    if (value === 'all') {
      setSelectedMapFilters(switchValues);
    } else {
      setSelectedMapFilters((prev) => prev.filter((v) => v !== value));
    }
  };
  ...
  const renderValue = (selected) => {
    ...
    const firstLabel = MAP_VIEW_FILTERS.find((opt) => opt.value === selected[0])?.label;
