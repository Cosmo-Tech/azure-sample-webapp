...
function makeKite({ tip, dir, perp, aLen, aWid }) {
  const base = { x: tip.x - dir.x * aLen, y: tip.y - dir.y * aLen };
  const mid = { x: tip.x - dir.x * (aLen * 0.55), y: tip.y - dir.y * (aLen * 0.55) };
  const left = { x: mid.x + perp.x * (aWid / 2), y: mid.y + perp.y * (aWid / 2) };
  const right = { x: mid.x - perp.x * (aWid / 2), y: mid.y - perp.y * (aWid / 2) };
  return [
    [tip.x, tip.y],
    [left.x, left.y],
    [base.x, base.y],
...
function makeChevron({ tip, dir, perp, aLen, aWid }) {
  const base = { x: tip.x - dir.x * aLen, y: tip.y - dir.y * aLen };
  const inner = { x: tip.x - dir.x * (aLen * 0.45), y: tip.y - dir.y * (aLen * 0.45) };
  const left = { x: inner.x + perp.x * (aWid / 2), y: inner.y + perp.y * (aWid / 2) };
  const right = { x: inner.x - perp.x * (aWid / 2), y: inner.y - perp.y * (aWid / 2) };
  return [
    [left.x, left.y],
    [tip.x, tip.y],
    [right.x, right.y],
...
function makeBarb({ tip, dir, perp, aLen, aWid }) {
  const base = { x: tip.x - dir.x * aLen, y: tip.y - dir.y * aLen };
  const notch = { x: tip.x - dir.x * (aLen * 0.55), y: tip.y - dir.y * (aLen * 0.55) };
  const left = { x: notch.x + perp.x * (aWid / 2), y: notch.y + perp.y * (aWid / 2) };
  const right = { x: notch.x - perp.x * (aWid / 2), y: notch.y - perp.y * (aWid / 2) };
  return [
    [tip.x, tip.y],
    [left.x, left.y],
    [base.x, base.y],
...
/**
 * Ensure a transport edge container exists, and return { container, update }.
 * Defaults mirror your old file, but use Pixi v8 Graphics API so strokes render.
 */
export function ensureTransportEdge(rootContainer, id) {
  ...
  function update({
    ...
    if (typeof arrowShape === 'function') {
      ...
      switch (arrowShape) {
        case 'kite':
          points = makeKite({ tip, dir, perp, aLen, aWid });
          break;
        case 'chevron':
          points = makeChevron({ tip, dir, perp, aLen, aWid });
          break;
        case 'barb':
          points = makeBarb({ tip, dir, perp, aLen, aWid });
          break;
        case 'triangle':
        default:
