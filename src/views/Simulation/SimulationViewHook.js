...
export const useSimulationView = () => {
  ...
  const lastIsDarkTheme = useRef(isDarkTheme);
  ...
  useEffect(() => {
    ...
    if (lastIsDarkTheme.current !== isDarkTheme) {
      lastIsDarkTheme.current = isDarkTheme;
      requiredUpdateStepsRef.current.theme = true;
    }
