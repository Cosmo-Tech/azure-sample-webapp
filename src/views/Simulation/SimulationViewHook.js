...
import { useCurrentScenarioId, useScenarios } from '../../state/hooks/ScenarioHooks';
...
export const useSimulationView = () => {
  ...
  // FIXME: temporary work-around, to be replaced by a new scenario selector with fake data in the Simulation view
  const scenarioIndex = scenarios.findIndex((scenario) => scenario.id === currentScenarioId);
  const fakeScenarioId = FAKE_SCENARIOS_METADATA[scenarioIndex % 3].lastRunId;
  const fakeScenario = FAKE_SCENARIOS_DATA[fakeScenarioId];
  ...
  useEffect(() => {
    ...
    graphRef.current = getGraphFromInstance(fakeScenario, settings);
  ...
  useEffect(() => {
    ...
    if (lastScenarioId.current !== currentScenarioId) {
      lastScenarioId.current = currentScenarioId;
