...
describe('compareAccessControlLists', () => {
  ...
  test('with identicals ACLs', () => {
    let { usersToAdd, usersToRemove } = SecurityUtils.compareAccessControlLists(aclA1, aclA1);
    expect(usersToAdd).toStrictEqual([]);
    expect(usersToRemove).toStrictEqual([]);
    ({ usersToAdd, usersToRemove } = SecurityUtils.compareAccessControlLists(aclA1, aclA2));
    expect(usersToAdd).toStrictEqual([]);
    expect(usersToRemove).toStrictEqual([]);
  });

  test('with different ACLs', () => {
    let { usersToAdd, usersToRemove } = SecurityUtils.compareAccessControlLists(aclA1, aclB);
    expect(usersToAdd).toStrictEqual([{ id: 'foo', role: 'anotherRole' }]);
    expect(usersToRemove).toStrictEqual([{ id: 'foo', role: 'role' }]);

    ({ usersToAdd, usersToRemove } = SecurityUtils.compareAccessControlLists(aclA1, aclC));
    expect(usersToAdd).toStrictEqual([{ id: 'baz', role: 'role' }]);
    expect(usersToRemove).toStrictEqual([]);

    ({ usersToAdd, usersToRemove } = SecurityUtils.compareAccessControlLists(aclA1, aclD));
    expect(usersToAdd).toStrictEqual([]);
    expect(usersToRemove).toStrictEqual([
      { id: 'foo', role: 'role' },
      { id: 'bar', role: 'role' },
