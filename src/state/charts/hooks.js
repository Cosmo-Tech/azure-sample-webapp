...
export const useCurrentSupersetDashboard = () => {
  ...
  return useMemo(() => {
    ...
    if (Array.isArray(scenarioView)) {
      const config = scenarioView.find((c) => c.runTemplateIds?.includes(runTemplateId));
      ...
      } else if (scenarioView.default) {
        dashboardId = scenarioView.default;
      }
    }
    ...
    const fullDashboard = dashboards.dashboards?.find((d) => d.id === dashboardId);
