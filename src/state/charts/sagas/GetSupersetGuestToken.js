...
function extractDashboardIds(chartsConfig) {
  ...
  if (chartsConfig?.scenarioView) {
    const scenarioView = chartsConfig.scenarioView;

    if (Array.isArray(scenarioView)) {
      scenarioView.forEach((dashboard) => {
        if (dashboard?.dashboardId) {
          dashboardIds.add(dashboard.dashboardId);
        }
      });
    } else if (typeof scenarioView === 'object') {
      if (scenarioView.default) {
        dashboardIds.add(scenarioView.default);
      }
      if (scenarioView.overrides) {
        Object.values(scenarioView.overrides).forEach((id) => {
          if (id) dashboardIds.add(id);
        });
      }
    }
  }

  const dashboardView = chartsConfig?.dashboardView || chartsConfig?.dashboardsView;
  if (dashboardView && Array.isArray(dashboardView)) {
    dashboardView.forEach((dashboard) => {
      if (dashboard?.id) {
        dashboardIds.add(dashboard.id);
      } else if (dashboard?.dashboardId) {
        dashboardIds.add(dashboard.dashboardId);
      }
    });
  }

  if (chartsConfig?.dashboards && Array.isArray(chartsConfig.dashboards)) {
    chartsConfig.dashboards.forEach((dashboard) => {
      if (dashboard?.id) {
        dashboardIds.add(dashboard.id);
      }
    });
  ...
  const ids = Array.from(dashboardIds);
  return ids;
}
