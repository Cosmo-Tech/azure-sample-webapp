...
export const LaunchButton = () => {
  ...
  const isCurrentScenarioDatasetUnavailable = useMemo(() => {
    if (!currentScenarioDatasetList || currentScenarioDatasetList?.length === 0) return true;
          ...
          title={
            isCurrentScenarioDatasetUnavailable
              ? t(
                  ...
                  "The scenario cannot be run because its dataset isn't found or has ingestion status ERROR"
                )
              : ''
          }
        >
