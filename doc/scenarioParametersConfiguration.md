...
In the Scenario view, the top half represents the parameters that users can modify before running a scenario. These
...
_Important: You can use this file to reorganize the scenario parameters tabs by changing the parameters belonging to parameters groups BUT **the set of parameters associated to a run template must remain consistent with the solution configuration file** (Solution.yaml)._
